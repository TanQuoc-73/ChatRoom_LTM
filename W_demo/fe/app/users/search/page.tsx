...
export default function UserSearchPage() {
  ...
  // Get avatar display
  const getAvatarDisplay = (user: typeof users[0]) => {
    if (user?.avatarUrl) {
      return (
        <img 
          src={user.avatarUrl} 
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-base text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
              />
              {isLoading && (
            ...
            {searchQuery && !isLoading && (
              <p className="mt-3 text-sm text-gray-600">
                {users.length > 0 ? (
                  <>Tìm thấy <span className="font-semibold">{users.length}</span> kết quả cho "<span className="font-semibold">{searchQuery}</span>"</>
                ) : error ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between">
                        <p className="text-sm text-gray-600">
                          {user.lastActive && (
                            <span>Hoạt động lần cuối: {formatDate(user.lastActive)}</span>
                          )}
                        </p>
          ...
          {!searchQuery && !isLoading && (
            <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-600">
                Nhập username hoặc tên hiển thị để tìm kiếm người dùng trong hệ thống
              </p>
            </div>
          )}

          {/* Empty State - No Results */}
          {searchQuery && users.length === 0 && !isLoading && !error && (
            <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
              <Search className="h-16 w-16 text-gray-300 mx-auto mb-4" strokeWidth={1} />
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Không tìm thấy kết quả</h3>
              <p className="text-gray-600">
                Không có người dùng nào với username hoặc tên hiển thị "{searchQuery}". Thử tìm kiếm với từ khóa khác.
              </p>
            </div>
          )}
        </div>
